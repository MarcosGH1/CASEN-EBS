...
model_data <- merged_data %>%
  mutate(
    ...
    zona = factor(zona, levels = c(1, 2), labels = c("Urbana", "Rural")),
    ...
    # Variable objetivo
    elite = factor(elite, levels = c(0, 1), labels = c("No", "Si"))
  )

# Verificar estructura preliminar
glimpse(model_data)
...
missing_vars <- setdiff(predictors, names(model_data))
if(length(missing_vars) > 0) {
  stop("Variables faltantes: ", paste(missing_vars, collapse = ", "))
}

# Seleccionar variables finales
model_data_final <- model_data %>%
  select(all_of(c(predictors, "elite")))

# Guardar datos procesados
saveRDS(model_data_final, "data/processed/model_data.rds")
...
Estratificación:
Se usa createDataPartition para mantener la proporción de la variable objetivo (9% elite)
La proporción 70-30 equilibra tamaño de entrenamiento y validación confiable.
...
Decisiones sobre hiperparámetros:
...
Monitoreo de representatividad regional:
...
# Crear índices de partición estratificada 
...
# Verificar distribución de la variable objetivo y mostrar con kable
...
rbind(train_dist, test_dist) %>%
  kable(caption = "Distribución de clases en conjuntos de entrenamiento y prueba",
...
region_dist <- data.frame(
  Entrenamiento = table(train_data$region),
  Prueba = table(test_data$region)
) %>%
...
Detección de Elite:
El modelo identifica correctamente al 97.6% de quienes realmente pertenecen al 10% superior
Este alto porcentaje sugiere que la elite económica tiene características distintivas muy marcadas La baja tasa de falsos negativos (2.4%) indica que es raro que el modelo "se pierda" a alguien del 10% superior
...
Los falsos positivos podrían representar un "grupo aspiracional" con capital cultural/educacional similar a la elite pero menor ingreso. La precisión territorial del modelo confirma la importancia de la segregación espacial.

Limitaciones y Alcances:
El modelo es mejor identificando quién sí pertenece a la elite que quién no. La tendencia a falsos positivos sugiere que las características asociadas a la elite no garantizan la pertenencia a ella.
...
El nivel educacional emerge como el predictor más potente, particularmente la distinción entre educación universitaria/postgrado y el resto. La región de residencia es el segundo factor más importante, reflejando la marcada concentración territorial de la elite. La edad muestra una importancia moderada, con un peak de probabilidad entre 30-40 años.

Falsos Negativos (3.75% de los casos):
Típicamente profesionales jóvenes con alta educación pero en regiones
Personas con postgrado pero en sectores académicos o públicos

Falsos Positivos (2.23% de los casos):
Concentrados en personas con educación universitaria en RM
Presentes en sectores con alta valoración educativa pero menores ingresos (educación, cultura)
...
# Matriz de confusión y convertir a kable
conf_matrix <- confusionMatrix(predictions, test_data$elite)
...
metrics <- data.frame(
  ...
  Valor = c(
    ...
    auc(roc(as.numeric(test_data$elite == "Si"), pred_probs[,"Si"]))
  )
)
...
# Curva ROC
roc_obj <- roc(as.numeric(test_data$elite == "Si"), pred_probs[,"Si"])
ggroc(roc_obj) +
  theme_minimal() +
  ggtitle(paste("Curva ROC (AUC =", round(auc(roc_obj), 3), ")")) +
  geom_abline(intercept = 1, slope = 1, linetype = "dashed", alpha = 0.8)

# Importancia de variables
var_imp <- vip(rf_model)
```

Patrones Territoriales de Error:
Región Metropolitana: Mayor concentración de falsos positivos.
Antofagasta, Tarapacá: Mayor proporción de falsos negativos.

La dominancia de factores educativos sugiere la persistencia de barreras educacionales
Los patrones territoriales reflejan la necesidad de desarrollo regional más equilibrado
Los falsos positivos indican potenciales "bolsones" de subvaloración profesional

Este análisis sugiere que la pertenencia al 10% superior en Chile está fuertemente estructurada por la combinación de educación y territorio, con la edad jugando un rol moderador. Los errores del modelo iluminan casos donde esta estructura se rompe, sea por rutas alternativas de acceso a altos ingresos o por barreras que impiden que el capital educativo se traduzca en retornos económicos equiparables.
...
Mayor bienestar subjetivo se asocia con mayor probabilidad de pertenecer al 10% superior
Los falsos positivos del modelo (personas predichas como elite pero que no lo son) tienden a mostrar niveles de bienestar similares a la elite real. La flexibilidad laboral y el control sobre el tiempo muestran una relación no lineal con la pertenencia al grupo elite.
...
# Mostrar resumen de errores con kable
kable(error_summary,
...
error_by_region <- error_analysis %>%
  mutate(region = test_data$region[id]) %>%
...
# Mostrar distribución de errores por región con kable
