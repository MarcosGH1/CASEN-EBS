...
- Identificar un conjunto de variables predictoras capaces de predecir la pertenencia al 10% superior de ingresos
...
- Generar recomendaciones basadas en los patrones identificados
...
vars_of_interest <- c("edad", "sexo", "zona", "region", "educ_rec", "tenencia_vivienda", 
                     "ytotcorh", "elite", "fexp")
...
gg_miss_var(merged_data[vars_of_interest]) +
  ...
  labs(title = "Patrón de valores faltantes en variables principales",
...
# Función para crear tablas combinadas
create_combined_table <- function(data, group_var, value_vars) {
  data %>%
    ...
    mutate(across(
      ends_with("_mean"),
      ~paste0(round(., 2), " (", round(get(str_replace(cur_column(), "_mean", "_sd")), 2), ")")
    )) %>%
...
##11. Resumen variables potencialmente predictoras
...
##12. Variables de bienestar de EBS
